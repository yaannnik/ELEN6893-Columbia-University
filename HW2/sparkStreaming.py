...
# global variables
bucket = "yy3089_data_storage" # TODO : replace with your own bucket name
...
def hashtagCount(words):
    ...
    words = words.filter(lambda x: (x != "") and (x[0] == "#") and re.match("^[A-Za-z0-9]+$", x[1:]) is not None) \
            .map(lambda x: (x.lower(),1)) \
            .updateStateByKey(lambda a, b: sum(a) + (b or 0))
    words = words.transform(lambda rdd: rdd.sortBy(lambda x: x[1], False))
...
def wordCount(words):
    ...
    words = words.filter(lambda x: (x != "") and (x.lower() in WORD)) \
            .map(lambda x: (x.lower(), 1)) \
            .reduceByKeyAndWindow(lambda a, b: a + b, lambda a, b: a - b, 60, 60)
    words = words.transform(lambda rdd: rdd.sortBy(lambda x:x[1], False) \
                            .map(lambda x: (x[0], x[1], time.strftime("%Y-%m-%d %X"))))
...
if __name__ == '__main__':
    ...
    topTags.foreachRDD(lambda x: saveToStorage(x, output_directory_hashtags,
                                            columns_name_hashtags, "overwrite"))
    wordCount.foreachRDD(lambda x: saveToStorage(x, output_directory_wordcount,
                                            columns_name_wordcount, "append"))
